...
from drop.payment.backends.pay_on_delivery import PayOnDeliveryBackend
...
class GeneralPaymentBackendTestCase(TestCase):
    ...
    def test_enforcing_of_name_works(self):
        MODIFIERS = ['drop.tests.payment.MockPaymentBackend']
        with SettingsOverride(DROP_PAYMENT_BACKENDS=MODIFIERS):
            self.assertRaises(NotImplementedError,
                backends_pool.get_payment_backends_list)

    def test_enforcing_of_namespace_works(self):

        MODIFIERS = ['drop.tests.payment.NamedMockPaymentBackend']
        with SettingsOverride(DROP_PAYMENT_BACKENDS=MODIFIERS):
            self.assertRaises(NotImplementedError,
                backends_pool.get_payment_backends_list)
    ...
    def test_get_order_returns_sensible_nulls(self):
        ...
        be = ValidMockPaymentBackend(drop=PaymentAPI())
        order = be.drop.get_order(MockRequest())
        self.assertEqual(order, None)

    def test_get_backends_from_pool(self):
        MODIFIERS = ['drop.tests.payment.ValidMockPaymentBackend']
        with SettingsOverride(DROP_PAYMENT_BACKENDS=MODIFIERS):
    ...
    def test_get_backends_from_non_class(self):
        MODIFIERS = ['drop.tests.payment.IdontExistEither']
        with SettingsOverride(DROP_PAYMENT_BACKENDS=MODIFIERS):
            self.assertRaises(ImproperlyConfigured,
                backends_pool.get_payment_backends_list)
...
class PayOnDeliveryTestCase(TestCase):
    ...
    def test01_backend_returns_urls(self):
        be = PayOnDeliveryBackend(drop=PaymentAPI())
        urls = be.get_urls()
    ...
    def test_order_required_before_payment(self):
        ...
        self.assertEqual('http://testserver/drop/cart/', response._headers['location'][1])
